...
async function sendTelegramNotification(booking) {
    const formattedDate = formatDateTime(booking.datetime);
    const message = `
...
3. Body area to focus: ${booking.focus_area || 'None'}
   Body area to avoid: ${booking.avoid_area || 'None'}
...
`;
    try {
        ...
        console.log('✅ Telegram alert sent with new remark fields.');
    } catch (error) {
...
app.post('/booking_spa12', async (req, res) => {
    ...
    console.log('Received data:', { service, therapyName, duration, price, name, phone, datetime, aromaOil, pressure, focusArea, avoidArea });
    console.log('-------------------------------------');
    // --- END NEW LOGGING ---
    ...
    // // ✅ Restrict to booking between 10:00 AM and 10:00 PM
    // const hour = bookingStart.getHours();
    // if (hour < 10 || hour >= 22) {
    //   return res.status(400).json({
    //     error: "Booking time must be between 10:00 AM and 10:00 PM."
    //   });
    // }
    ...
    try {
        ...
        // --- NEW LOGGING ---
        console.log('--- Booking Data After Database Insertion (result.rows[0]) ---');
